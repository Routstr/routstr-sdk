...
describe("manual vs auto disabled providers — ProviderRegistry", () => {
  it("getDisabledProviders on registry returns union", async () => {
    const driver = createMemoryDriver();
    const { store } = createSdkStore({ driver });
    const registry = createProviderRegistryFromStore(store);
    ...
    expect(disabled).toContain("https://review-bad.example.com/");
    expect(disabled).toContain("https://manual-bad.example.com/");
  });
});
